...
fun MainScreen(
    ...
    onSessionInfoProvided: (String, Int) -> Unit = { _, _ -> },
    onDismissSessionDialog: () -> Unit = {}
) {
    Log.i(TAG, "MainScreen Composable")
    HRDataTransferTheme {
        // Show session input dialog when requested
        SessionInputDialog(
            onSessionInfoProvided = onSessionInfoProvided,
        ...
        Row(
            ...
            Column(
                ...
                Box(
                    ...
                    Button(
                        onClick = {
                            // When running, we simply stop
                            // When not running, onStart will show the session dialog first
                            startButtonPressed = if (trackingRunning) {
                                onStop.invoke()
                                false
                            } else {
                                onStart.invoke() // This will trigger the session dialog
                                true
                            }
                        },
